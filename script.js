...
function createKeyboardLanguage() {
  if (storage.getItem("language") === null) {
    ...
    currentLanguageCodes = keyCodeActionEnglish;
  }
}
...
function loadCurrentKeyboardCodes() {
  if (storage.getItem("language") === "en") {
    ...
    currentLanguageCodes = keyCodeActionRussian;
  }
}
...
function clickKeyAnimation() {
  ...
  window.addEventListener("blur", () => {
    for (let key in keyCodesElements) {
      document.querySelector(keyCodesElements[key]).classList.remove("clicked");
    }
  });
}
...
function changeLanguage() {
  function changeToRussian() {
    ...
    document.getElementById("keyboard").innerHTML = russianLayout;
    // if (capslock === true) {
    //   for (let elem of document.querySelectorAll('.container > div:first-child > div:first-child, .container > div:nth-child(2) > div, .container > div:nth-child(3) > div, .container > div:nth-child(4) > div')) {
...
function pasteSymbolToTextarea() {
  ...
  document.addEventListener("keydown", (event) => {
    if (event.code === "Backspace") {
      document.getElementById("textfield").textContent = document.getElementById("textfield").textContent.slice(0, -1);
    } else if (event.code in currentLanguageCodes && capslock === true && isLetter(currentLanguageCodes[event.code])) {
      document.getElementById("textfield").textContent += currentLanguageCodes[event.code].toUpperCase();
    } else if (event.code in currentLanguageCodes) {
      document.getElementById("textfield").innerHTML += currentLanguageCodes[event.code];
    }
  });
}
...
function clickByMouse() {
  ...
  function pasteSymbolToTextarea(value) {
    function isLetter(sign) {
      if (storage.getItem("language") === "en") {
        return /[a-z]/i.test(sign);
      } else if (storage.getItem("language") === "ru") {
      ...
      if (value === "Backspace") {
        document.getElementById("textfield").textContent = document.getElementById("textfield").textContent.slice(0, -1);
      } else if (value in currentLanguageCodes && capslock === true && isLetter(currentLanguageCodes[value])) {
        document.getElementById("textfield").textContent += currentLanguageCodes[value].toUpperCase();
      } else if (value in currentLanguageCodes) {
        document.getElementById("textfield").innerHTML += currentLanguageCodes[value];
      }
  ...
  document.getElementById("keyboard").addEventListener("mousedown", (event) => {
    ...
    if (clicked.classList.contains("button")) {
      for (let value in currentLanguageCodes) {
        if (clicked.classList.contains(value)) {
          pasteSymbolToTextarea(value)
